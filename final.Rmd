...
Bifurcation Diagram: Predation Rate(C)
```{r}
C_seq <- seq(0.001, 0.1, length.out = 200)
R_eq <- numeric(length(C_seq))
P_eq <- numeric(length(C_seq))
T_eq <- numeric(length(C_seq))
for(i in 1:length(C_seq)){

  C_temp <- C_seq[i]

  modelA_temp <- function(time, state, parameters){
...
plot(C_seq, R_eq,
     ...
     col = "blue",
     lwd = 2,
     xlab = "Predation rate (C)",
     ylab = "Equilibrium population",
     main = "Residents (R)")
